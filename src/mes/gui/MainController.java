...
public class MainController implements Initializable {
    ...
    private void handleSelectOrder(ActionEvent event) {
        ...
        for(Status st : this.orderMes.fetchStatuses()) {
            if(st.getId() != 1) {
        ...
        if (currentOrder != null) {
            ...
            if (currentOrder.getStatus().getValue().equals("Færdig")) {
                ...
                int pbId = prodBlockMes.getSuggestedProductionBlock();
                if(pbId == -1) {
                    btnPrepOrder.setDisable(true);
    ...
    private void handleDeleteSequence(ActionEvent event) {
        if (tableViewSequences.getSelectionModel().getSelectedItem() != null) {
            this.currentGrowthProfile.getLightSequence().remove(tableViewSequences.getSelectionModel().getSelectedIndex());

            tableViewSequences.getItems().clear();
            this.setGpLightTableView(this.currentGrowthProfile.getLightSequence());
        }
    }
